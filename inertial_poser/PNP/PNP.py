...
from inertial_poser.PNP.dynamics import PhysicsOptimizer
...
class PNP(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.to(self.device).load_state_dict(torch.load('inertial_poser/PNP/weights/weights.pt', map_location=self.device))
    ...
    @torch.no_grad()
    def forward_frame(self, a, w, R, only_pose=False):
        R = R.view(6, 3, 3)
        RIR = R[5]
